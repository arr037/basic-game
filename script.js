...
function createConfetti() {
  ...
  confettiContainer.innerHTML = ""; // Очищаем предыдущие элементы конфетти

  for (let i = 0; i < 100; i++) {
    // Создаем 100 элементов конфетти
    const confettiPiece = document.createElement("div");
    confettiPiece.style.backgroundColor = `hsl(${
      Math.random() * 360
    }, 70%, 50%)`; // Случайный цвет
    confettiPiece.style.position = "absolute";
    confettiPiece.style.width = "10px";
    confettiPiece.style.height = "10px";
    confettiPiece.style.left = `${Math.random() * 100}vw`;
    confettiPiece.style.animationDuration = `${Math.random() * 3 + 2}s`; // Случайная продолжительность
    confettiPiece.style.animationName = "fall";
    confettiPiece.style.opacity = Math.random(); // Случайная прозрачность
